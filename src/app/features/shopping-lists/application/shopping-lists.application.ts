...
import { ShoppingListDetailResult, ShoppingListGateway } from './ports/shopping-list.gateway';

export class ShoppingListsApplication {
  constructor(private readonly gateway: ShoppingListGateway) {}
  list(householdId: string, trashed = false): Observable<readonly ShoppingListSummary[]> { return this.gateway.list(householdId, trashed); }
  ...
  addItem(householdId: string, listId: string, draft: ShoppingItemDraft): Observable<ShoppingItem> { return this.gateway.addItem(householdId, listId, draft); }
