...
import { ShoppingListDetailResult, ShoppingListGateway } from '../../application/ports/shopping-list.gateway';
...
export class HttpShoppingListGateway implements ShoppingListGateway {
  ...
  list(householdId: string, trashed = false): Observable<readonly ShoppingListSummary[]> {
    const params = trashed ? new HttpParams().set('trashed', true) : undefined;
    return this.http.get<unknown>(this.collection(householdId), { params }).pipe(map(mapShoppingLists), this.errors());
  }
  ...
  addItem(householdId: string, listId: string, draft: ShoppingItemDraft): Observable<ShoppingItem> { return this.http.post<unknown>(`${this.collection(householdId)}/${listId}/items`, this.itemBody(draft)).pipe(map(mapShoppingItem), this.errors()); }
