...
import { Expense, ExpenseClassificationChange, Money } from '../../domain';
...
const optionalNullableString = (value: unknown): string | null => value === undefined || value === null ? null : string(value);
...
export function mapExpense(value: unknown): Expense {
  ...
  return {
    ...
    payerMemberId: string(dto['payerMemberId']), categoryId: optionalNullableString(dto['categoryId']), amount: Money.fromDecimal(string(dto['amount']), currency),
...
export function mapClassificationHistory(value: unknown): readonly ExpenseClassificationChange[] {
  if (!Array.isArray(value)) return invalid();
  return value.map(item => { const dto = object(item); return {
    id: string(dto['id']), expenseId: string(dto['expenseId']), changedByMemberId: string(dto['changedByMemberId']),
    previousCategoryId: optionalNullableString(dto['previousCategoryId']), newCategoryId: optionalNullableString(dto['newCategoryId']),
    reason: optionalNullableString(dto['reason']), changedAt: string(dto['changedAt']),
  }; });
}
